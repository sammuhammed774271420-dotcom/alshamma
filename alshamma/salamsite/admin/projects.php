...
<div class="page-actions">
    <h3 style="font-size:16px;font-weight:700;color:var(--dark);"><?php echo count($projects); ?> منتج</h3>
    <a href="/admin/project-form.php" class="btn btn-gold"><i class="fas fa-plus"></i> إضافة منتج جديد</a>
</div>

<div class="admin-card">
    <div class="admin-card-body" style="padding:0;">
        <?php if (empty($projects)): ?>
        <div class="empty-state"><i class="fas fa-bread-slice"></i><p>لا توجد منتجات. ابدأ بإضافة منتج جديد.</p></div>
        <?php else: ?>
        <table class="admin-table">
            <thead><tr><th>الصورة</th><th>الاسم</th><th>التصنيف</th><th>الحالة</th><th>السعر</th><th>مميز</th><th>الإجراءات</th></tr></thead>
            <tbody>
            <?php foreach ($projects as $p): ?>
            <tr>
                <td><?php if (!empty($p['image_path']) && file_exists(__DIR__.'/../'.$p['image_path'])): ?><img src="/<?php echo htmlspecialchars($p['image_path']); ?>" alt=""><?php else: ?><div style="width:50px;height:40px;background:#eee;border-radius:4px;display:flex;align-items:center;justify-content:center;"><i class="fas fa-bread-slice" style="color:#ccc;"></i></div><?php endif; ?></td>
