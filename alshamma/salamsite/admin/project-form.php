...
$id = (int)($_GET['id'] ?? 0);
$project = ['name'=>'','location'=>'','status'=>'متاح','description'=>'','price'=>'','area'=>'','added_date'=>date('Y-m-d'),'featured'=>0,'image_path'=>'','gallery'=>'[]'];
...
if ($id) {
    ...
    $admin_title = 'تعديل منتج';
} else {
    $admin_title = 'إضافة منتج جديد';
}
$admin_icon = 'box-open';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) { $error = 'اسم المنتج مطلوب'; }
    else {
...
<div class="admin-card">
    <div class="admin-card-header">
        <h3><i class="fas fa-box-open"></i> <?php echo $admin_title; ?></h3>
    ...
    <div class="admin-card-body">
        <form method="POST" enctype="multipart/form-data">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                <div>
                    ...
                    <div class="form-mb">
                        ...
                        <input type="text" name="location" class="form-control" value="<?php echo htmlspecialchars($project['location'] ?? ''); ?>" placeholder="مثال: خبز عربي، معجنات، حلويات">
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                        <div class="form-mb">
                            <label class="form-label">حالة المنتج</label>
                            <select name="status" class="form-control">
                                <?php foreach(['متاح','غير متاح','قريباً','موسمي','الأكثر مبيعاً'] as $st): ?>
                        ...
                        <div class="form-mb">
                            ...
                            <input type="text" name="price" class="form-control" value="<?php echo htmlspecialchars($project['price'] ?? ''); ?>" placeholder="مثال: 0.500 د.أ">
                        </div>
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                        <div class="form-mb">
                            <label class="form-label">الوزن / الحجم</label>
                            <input type="text" name="area" class="form-control" value="<?php echo htmlspecialchars($project['area'] ?? ''); ?>" placeholder="مثال: 500 غرام">
                        </div>
                        <div class="form-mb">
            ...
            <div style="display:flex;gap:10px;padding-top:15px;border-top:1px solid #eee;margin-top:5px;">
                <button type="submit" class="btn btn-gold"><i class="fas fa-save"></i> <?php echo $id ? 'تحديث المنتج' : 'إضافة المنتج'; ?></button>
